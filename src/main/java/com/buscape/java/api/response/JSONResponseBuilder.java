...
public class JSONResponseBuilder extends ResponseBuilder {
	...
	public ResponseBuilder parse(String content) {
		try {
			...
			if (json.has("advertiser")) {
				final JSONArray jsonAdvertiserArray = json
						.getJSONArray("advertiser");
				final List<Advertiser> advertiserList = new ArrayList<Advertiser>();

				for (i = 0, t = jsonAdvertiserArray.length(); i < t; ++i) {
					advertiserList.add(parseAdvertiser(jsonAdvertiserArray
							.getJSONObject(i).getJSONObject("advertiser")));
				}
			...
			if (json.has("offer")) {
				...
				for (i = 0, t = jsonOfferArray.length(); i < t; ++i) {
					offerList.add(parseOffer(jsonOfferArray.getJSONObject(i)
							.getJSONObject("offer")));
				}
			...
			if (json.has("product")) {
				...
				for (i = 0, t = jsonProductArray.length(); i < t; ++i) {
					productList.add(parseProduct(jsonProductArray
			...
			if (json.has("subcategory")) {
				final JSONArray jsonSubcategoryArray = json
				...
				for (i = 0, t = jsonSubcategoryArray.length(); i < t; ++i) {
					subCategoryList.add(parseCategory(jsonSubcategoryArray
							.getJSONObject(i).getJSONObject("subcategory")));
				}
			...
			if (json.has("top5category")) {
				final JSONArray jsonTop5categoryArray = json
						.getJSONArray("top5category");
				final List<Category> top5CategoryList = new ArrayList<Category>();

				for (i = 0, t = jsonTop5categoryArray.length(); i < t; ++i) {
					top5CategoryList.add(parseCategory(jsonTop5categoryArray
							.getJSONObject(i).getJSONObject("top5category")));
				}
			...
			if (json.has("totalresultsavailable")) {
				result.setTotalResultsAvailable(json
						.getInt("totalresultsavailable"));
			}

			if (json.has("totalresultssellers")) {
				result.setTotalResultsSellers(json
						.getInt("totalresultssellers"));
			}

			if (json.has("totalresultsreturned")) {
				result.setTotalResultsReturned(json
	...
	private List<Address> parseAddresses(JSONArray addresses)
	...
	private List<Contact> parseContacts(JSONArray contacts)
			throws JSONException {
		final List<Contact> contactList = new ArrayList<Contact>();

		for (int i = 0, t = contacts.length(); i < t; ++i) {
			final JSONObject jsonContact = (JSONObject) contacts.getJSONObject(
					i).get("contact");
	...
	private List<Link> parseLinks(JSONArray links) throws JSONException {
		...
		for (int i = 0, t = links.length(); i < t; ++i) {
			...
			final JSONObject jsonLink = links.getJSONObject(i).getJSONObject(
					"link");
	...
	private Product parseProduct(JSONObject p) throws JSONException {
		...
		if (p.has("specification")) {
			try {
				product.setSpecification(parseSpecification(p
						.getJSONObject("specification")));
	...
	private Rating parseRating(JSONObject r) throws JSONException {
		...
		rating.setUserAverageRating(parseUserAverageRating(r
				.getJSONObject("useraveragerating")));
	...
	private Specification parseSpecification(JSONObject s) throws JSONException {
		...
		for (int i = 0, t = jsonItemArray.length(); i < t; ++i) {
			specification.addItem(parseItem(jsonItemArray.getJSONObject(i)
					.getJSONObject("item")));
	...
	private UserAverageRating parseUserAverageRating(JSONObject u)
			throws JSONException {
		...
		if (u.has("specification")) {
			userAverageRating.setSpecification(parseSpecification(u
					.getJSONObject("specification")));
		}
